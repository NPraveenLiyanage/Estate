...
import {updateUserStart, updateUserSuccess, updateUserFalure, 
  deleteUserFalure, deleteUserStart, deleteUserSuccess, 
  signOutUserStart, signOutUserFalure, signOutUserSuccess} from "../redux/user/userSlice.js"
import { useDispatch } from "react-redux";
import { Link } from "react-router-dom";
import Listing from "../../../backend/models/listing.model.js";
...
export default function Profile() {
  ...
  const handleListingDelete = async(lisingId) => {
    try {
      const res = await fetch(`backend/listing/delete/${lisingId}`,{
        method: 'DELETE',
      });
      const data = await res.json();
      if(data.success == false) {
      ...
      // we get all listing except thia lisingId
      setuserListings((prev) => prev.filter((Listing) => Listing._id !== lisingId)); 
    } catch (error) {
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <p className="text-red-500 mt-5">{showListingError ? 'Error showing listing': ''}</p>
