    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-6'>
        <div className='flex flex-col flex-1 gap-4'>
          ...
          id='name' maxLength='64' minLength='10' onChange={handleChange} value={formData.name} required/>
           
           <textarea type='text' placeholder='Description' className='border p-3 rounded-lg'
          id='description' onChange={handleChange} value={formData.description} required/>
          ...
          <div className='flex flex-wrap gap-6'>
            <div className='flex items-center gap-2'>
              <input type="number" name="bedrooms" id="bedrooms" min='1' max='10' required
              className='p-3 border-green-700 rounded-lg' onChange={handleChange} value={formData.bedrooms}/>
              <p>Bedrooms</p>
            </div>
            <div className='flex items-center gap-2'>
              <input type="number" name="bathrooms" id="bathrooms" min='1' max='10' required
              className='p-3 border-green-700 rounded-lg' onChange={handleChange} value={formData.bathrooms}/>
              <p>Bathrooms</p>
            </div>
            <div className='flex items-center gap-2'>
              <input type="number" name="regularPrice" id="regularPrice" min='50' max='1000000' required
              className='p-3 border-green-700 rounded-lg' onChange={handleChange} value={formData.regularPrice}/>
              <div className='flex flex-col items-center'>
                <p>Regular Price</p>
                <span className='text-xs'>($ / months)</span>
              </div>
            </div>
            {formData.offer &&( // this will occur only this is a offer
              <div className='flex items-center gap-2'>
              <input type="number" name="discountPrice" id="discountPrice" min='0' max='10000000' required
              className='p-3 border-green-700 rounded-lg' onChange={handleChange} value={formData.discountPrice}/>
              <div className='flex flex-col items-center'>
                <p>Discount</p>
                <span className='text-xs'>($ / months)</span>
              </div>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          <div className="flex gap-3">
            <input onChange={(e) =>setFiles(e.target.files)} type="file" className='p-3 border border-gray-300 rounded w-full' name="images" id="images" accept='image/*' multiple />
            <button disabled={uploading} type='button' onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded
